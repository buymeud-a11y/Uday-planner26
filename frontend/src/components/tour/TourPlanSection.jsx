...
const TourPlanSection = ({ tourData }) => (
  <section className="py-16 px-4" style={{ background: "#FAF9F6" }} data-testid="tour-plan-section">
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-8">
        {tourData.best_time_to_visit && (
          <div className="px-5 py-2 rounded-full text-sm" style={{ background: "#F2ECE4", color: "#4A5A50" }}>
            Best time: <span className="font-medium" style={{ color: "#1C3325" }}>{tourData.best_time_to_visit}</span>
          </div>
        )}
        {tourData.estimated_total_food_cost_inr > 0 && (
          <div className="px-5 py-2 rounded-full text-sm" style={{ background: "#F2ECE4", color: "#4A5A50" }}>
            Est. food: <span className="font-medium" style={{ color: "#1C3325" }}>{formatINR(tourData.estimated_total_food_cost_inr)}</span>
          </div>
        )}
        <div className="px-5 py-2 rounded-full text-sm" style={{ background: "#F2ECE4", color: "#4A5A50" }}>
      ...
      {tourData.highlights?.length > 0 && (
        <div className="rounded-2xl p-6 mb-8" style={{ background: "#1C3325" }}>
          ...
          <div className="flex flex-wrap gap-2">
            {tourData.highlights.map((h, i) => (
              ...
              >
                <Star size={11} fill="currentColor" style={{ color: "#D99C42" }} />
                {h}
              </div>
            ))}
          </div>
      ...
      {tourData.travel_tips?.length > 0 && (
        <div className="mt-8 rounded-2xl p-6" style={{ background: "#F2ECE4" }}>
          <div className="flex items-center gap-2 mb-4">
            <Lightbulb size={18} style={{ color: "#D96B42" }} strokeWidth={1.5} />
            <h3 className="text-base font-medium" style={{ color: "#1C3325", fontFamily: "Outfit, sans-serif" }}>
              Travel Tips
            </h3>
          </div>
          <ul className="space-y-2">
            {tourData.travel_tips.map((tip, i) => (
              <li key={i} className="flex items-start gap-2 text-sm" style={{ color: "#4A5A50" }}>
                <span className="font-bold mt-0.5" style={{ color: "#D96B42" }}>•</span>
                {tip}
              </li>
            ))}
          </ul>
